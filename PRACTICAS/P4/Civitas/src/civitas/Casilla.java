...
public class Casilla {
    ...
    // Contructor de casilla de tipo Control Policial donde el modificador de la multa se pasa como parametro
    Casilla (TipoCasilla tipo, String nombre, float multa){
        this.tipo = tipo;
        this.nombre = nombre;
    ...
    void recibeJugador(int actual, ArrayList<Jugador> todos){
       switch(this.tipo){
            ...
            case CONTROL_POLICIAL:
                recibeJugador_controlPolicial(actual, todos);
                break;
            // FIN EXAMEN
            case DESCANSO: 
    ...
    private float getValorMulta(){
        return this.valorMulta;
    }
    
    // Metodo para comprobar si el jugador ha superado el límite de velocidad
    void recibeJugador_controlPolicial(int actual, ArrayList<Jugador> todos){
        this.informe(actual, todos); // informa de que el jugador ha caido 
        Jugador jugador = todos.get(actual); // obtenemos el jugador actual sobre el que se va a aplicar la multa
        int ultimaTirada = Dado.instance.getUltimoResultado(); // Obtenemos la última tirada del dado, para ver si se aplica la multa
        float multa = 0;
        if(ultimaTirada >= 3){ // si la ultima tirada es menor que 3, no se le aplica multa. Si es igual o mayor si
            multa = -(this.getValorMulta() * ultimaTirada); // calculamos el valor de la multa y la pongo en negativo para que le reste el valor al saldo del jugador 
            jugador.controlPolicial(actual, todos, multa ); // llamamos al metodo de jugador para aplicar la multa
        }
    }
    // FIN EXAMEN
